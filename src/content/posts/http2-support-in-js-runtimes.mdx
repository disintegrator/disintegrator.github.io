...
The Fetch API is supported in Node.js through [Undici][undici], the official HTTP client that is built into Node.js since at least Node.js v18. Out of the box, Node.js throws an error:
...
Sadly, Bun does not appear to support HTTP/2 with its fetch implementation at this time and I don't believe there are great workarounds at this time. I tried testing it with `undici` without success. There is however an [open issue][bun-http2-issue] opened by the Jarred, creator of Bun, to add HTTP/2 support.
...
For some time now, I've been optimistic about [WinterTC][winter-tc], an initiative that aims to formally define interoperability across server-side JavaScript runtimes. One of the goals is to properly define the requirements for the Fetch API on the server. However, as a quiet observer, it seems that things are moving slower than expected but I remain optimistic knowing the folks involved with it.
