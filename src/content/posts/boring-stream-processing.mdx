---
title: "Benthos: Stream processing… but make it boring"
...
> Take messages from a PubSub topic, format their timestamps to RFC3339, batch these up into a JSON array of 200 entries and POST them to an HTTP endpoint with a rate limit.

```yaml
input:
  gcp_pubsub:
    project: demo-gcp-project
    subscription: demo-pubsub-subscription

# Let's imagine messages coming from the topic have this format:
# {
#   "id": "abc123"
#   "ts": 1682852776
#   "event": "payment_processed",
#   "actor": "acme://merchants/12fa4", 
#   "subject": "acme://payments/ae1342",
#   "data": {"amount": "28.99","currency":"usd"}
# }

pipeline:
  processors:
    mapping: |
      # 1. cherry-pick what we want from the pubsub message
      root = this.with("id", "event", "actor", "subject")

      # 2. format the timestamp
      root.ts = this.ts.format("2006-01-02T15:04:05Z07:00")

      # 3. set the value field from data in the message
      root.value = this.data.amount

      # 4. derive the product code
      let event = this.event
      let currency = this.data.currency
      root.product_code = match this.event {
        "payment_processed" => "payment_" + $currency
        "payment_chargedback" => "chargeback_" + $currency
        _ => "unknown"
      }

output:
  http_client:
    url: "https://api.example.com/billing/usage"
    method: POST
    oauth2:
      enabled: true
    ...
    batching:
      count: 200
      period: 5s
      processors: # This series of processors runs when a batch is cut
        - label: collapse_batch
          archive:
            format: json_array
        - label: add_envelope
          mapping: |
